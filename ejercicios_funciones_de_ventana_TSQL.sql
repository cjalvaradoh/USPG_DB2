...
SELECT
    ...
    s.SalesAmount,
    s.SalesDate,
    YEAR(s.SalesDate) AS Anio,
    MONTH(s.SalesDate) AS Mes,
    t.TotalMensualVendedor,
    DENSE_RANK() OVER (
        PARTITION BY s.SalesRegion, YEAR(s.SalesDate), MONTH(s.SalesDate)
        ORDER BY t.TotalMensualVendedor DESC
    ) AS RankingVendedor,
    NTILE(4) OVER (
        PARTITION BY s.SalesRegion, YEAR(s.SalesDate), MONTH(s.SalesDate)
        ORDER BY t.TotalMensualVendedor DESC
...
FROM (
    SELECT
        ...
        SUM(SalesAmount) OVER (
            PARTITION BY SalesRegion, YEAR(SalesDate), MONTH(SalesDate), SalesPersonName
        ) AS TotalMensualVendedor
    FROM stg.sales
) AS t
INNER JOIN stg.sales s
    ON s.SalesID = t.SalesID;
